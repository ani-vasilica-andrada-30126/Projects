...
CREATE TABLE Imprumuturi (IDCarte int,IDSediu int references Sediu,NrCard int references Debitori,Data_imprumut date,Data_retur date);
...
VALUES
	  ...
	  (10,4,4,'2016-09-18','2016-10-18'),
	  (20,5,5,'2016-09-12','2016-10-12');
...
--proceduri stocate
CREATE PROCEDURE Nr_copii_sediu                 --procedura care selecteaza numarul de copii al unei carti dintr-un sediu  
...
CREATE PROCEDURE carti_autor_sediu             --procedura care selecteaza  numele cartilor unui autor dintr-un sediu 
...
WHERE A.NumeAutor=@Autor AND S.NumeSediu=@Sediu
 ...
 CREATE PROCEDURE debitori_imprumut_carte     --procedura care selecteaza numele debitorilor care au imprumutat o anumita carte 
 @ID int
 AS
 SELECT D.Nume
...
WHERE I.IDCarte=@ID
 ...
 exec debitori_imprumut_carte 1

--functii	
CREATE FUNCTION f_nr_carti_publicate_editura(@NumeEditura varchar(30)) -- functie pentru returnarea numarului de carti publicate de catre o editura  
RETURNS int
AS
BEGIN
...
CREATE FUNCTION f_titlu_carte_dupa_id(@IDCarte int) --functie pentru returnarea titlului unei carti dupa ID   
RETURNS varchar(30)
AS
BEGIN
...
CREATE FUNCTION f_nr_carti_sediu(@IDSediu int) -- functie pentru returnarea numarului de carti dintr-un anume sediu  
RETURNS int
AS
BEGIN
DECLARE @nr_carti int;
...
--triggere
CREATE TRIGGER trigger1 on Autori for update as  --nu se poate actualiza numele autorului   
if(UPDATE(NumeAutor))
Begin
...
CREATE TRIGGER trigger2 on Imprumuturi for delete --nu se pot sterge mai multe imprumuturi in acelasi timp   
as
Begin
...
where IDCarte in (1,2,3,4,5)


Create Trigger trigger3 on Debitori for update as          --nu se poate updata numele si adresa unui debitor in acelasi timp  
Begin
...
--Cursor
DECLARE cursor_1 CURSOR             --cursor care afiseaza data de returnare pt cartile imprumutate de catre fiecare debitor  
FOR SELECT
    IDCarte,
	NrCard,
	Data_retur
...
DECLARE cursor_2 CURSOR              --cursor care afiseaza id-ul cartilor care nu mai sunt in stoc la un anumit sediu      
FOR SELECT
    IDCarte,
	IDSediu,
	NrCopii
...
DECLARE cursor_3 CURSOR            --cursor care afiseaza cartile publicate de catre editura Libris                 
FOR SELECT
Titlu,
NumeEditura
from Carti
